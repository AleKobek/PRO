...
public class WiadomosciControllerTests
{
    private readonly Mock<IWiadomosciService> _mockWiadomoscService;
    ...
    private readonly WiadomosciController _controller;

    public WiadomosciControllerTests()
    {
        _mockWiadomoscService = new Mock<IWiadomosciService>();
        _mockStatystykiCzatuService = new Mock<IStatystykiCzatuService>();
        _mockUserManager = MockUserManager<Uzytkownik>();
        _controller = new WiadomosciController(
    ...
    public async Task GetWiadomosci_WithValidRecipientId_ReturnsOkWithMessages()
    {
        ...
        var actionResult = await _controller.GetWiadomosciPrywatne(2);
    ...
    public async Task CreateWiadomosc_WithValidData_ReturnsCreated()
    {
        ...
        _mockWiadomoscService
            .Setup(s => s.CreateWiadomoscPrywatna(2, "Hello!", user.Id))
            .ReturnsAsync(ServiceResult<bool>.Ok(true, 201));

        var actionResult = await _controller.CreateWiadomoscPrywatna(2, "Hello!");
    ...
    public async Task CreateWiadomosc_WithInvalidData_ReturnsBadRequest()
    {
        ...
        _mockWiadomoscService
            .Setup(s => s.CreateWiadomoscPrywatna(2, "", user.Id))
            .ReturnsAsync(ServiceResult<bool>.Fail(400, new[] { new ErrorItem("Message content is required", "Tresc") }));

        var actionResult = await _controller.CreateWiadomoscPrywatna(2,"");
