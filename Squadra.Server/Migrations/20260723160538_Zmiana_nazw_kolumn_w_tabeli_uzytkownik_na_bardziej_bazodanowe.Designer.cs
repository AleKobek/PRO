// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Squadra.Server.Context;

#nullable disable

namespace Squadra.Server.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260723160538_Zmiana_nazw_kolumn_w_tabeli_uzytkownik_na_bardziej_bazodanowe")]
    partial class Zmiana_nazw_kolumn_w_tabeli_uzytkownik_na_bardziej_bazodanowe
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole<int>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("Roles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<int>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("RoleId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("RoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<int>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("UserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<int>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("UserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<int>", b =>
                {
                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<int>("RoleId")
                        .HasColumnType("int");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("UserRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<int>", b =>
                {
                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("UserTokens", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.BibliotekaGier.Models.GraUzytkownika", b =>
                {
                    b.Property<int>("UzytkownikId")
                        .HasColumnType("int")
                        .HasColumnName("id_uzytkownika");

                    b.Property<int>("GraId")
                        .HasColumnType("int")
                        .HasColumnName("id_gry");

                    b.HasKey("UzytkownikId", "GraId")
                        .HasName("id_gra_uzytkownika");

                    b.HasIndex("GraId");

                    b.ToTable("Gra_uzytkownika", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.BibliotekaGier.Models.GraUzytkownikaNaPlatformie", b =>
                {
                    b.Property<int>("UzytkownikId")
                        .HasColumnType("int")
                        .HasColumnName("id_uzytkownika");

                    b.Property<int>("GraId")
                        .HasColumnType("int")
                        .HasColumnName("id_gry");

                    b.Property<int>("PlatformaId")
                        .HasColumnType("int")
                        .HasColumnName("id_platformy");

                    b.HasKey("UzytkownikId", "GraId", "PlatformaId")
                        .HasName("id_gra_uzytkownika_na_platformie");

                    b.HasIndex("PlatformaId");

                    b.ToTable("Gra_uzytkownika_na_platformie", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Drużyny.Models.Druzyna", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool>("CzyPubliczna")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true)
                        .HasColumnName("czy_publiczna");

                    b.Property<bool>("CzyZintegrowano")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false)
                        .HasColumnName("czy_zintegrowano");

                    b.Property<int>("GraId")
                        .HasColumnType("int")
                        .HasColumnName("id_gry");

                    b.Property<int>("KapitanId")
                        .HasColumnType("int")
                        .HasColumnName("id_kapitana");

                    b.Property<int>("NastrojRozgrywkiId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(1)
                        .HasColumnName("id_nastroju_rozgrywki");

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("nvarchar(40)")
                        .HasColumnName("nazwa");

                    b.Property<string>("Opis")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)")
                        .HasColumnName("opis");

                    b.Property<int?>("PlatformaId")
                        .HasColumnType("int")
                        .HasColumnName("id_platformy");

                    b.Property<int?>("WymaganyJezykId")
                        .HasColumnType("int")
                        .HasColumnName("id_wymaganego_jezyka");

                    b.Property<int?>("WymaganyStopienBieglosciJezykaId")
                        .HasColumnType("int")
                        .HasColumnName("id_wymaganego_stopnia_bieglosci_jezyka");

                    b.HasKey("Id");

                    b.HasIndex("GraId");

                    b.HasIndex("KapitanId");

                    b.HasIndex("NastrojRozgrywkiId");

                    b.HasIndex("PlatformaId");

                    b.HasIndex("WymaganyJezykId");

                    b.HasIndex("WymaganyStopienBieglosciJezykaId");

                    b.ToTable("Druzyna", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Drużyny.Models.MiejsceWDruzynie", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("DruzynaId")
                        .HasColumnType("int")
                        .HasColumnName("id_druzyny");

                    b.Property<DateTime?>("OstatnieOtwarcieCzatu")
                        .HasColumnType("datetime2")
                        .HasColumnName("ostatnie_otwarcie_czatu");

                    b.Property<int?>("RolaId")
                        .HasColumnType("int")
                        .HasColumnName("id_roli");

                    b.Property<int?>("StatystykaId")
                        .HasColumnType("int")
                        .HasColumnName("id_statystyki");

                    b.Property<int?>("UzytkownikId")
                        .HasColumnType("int")
                        .HasColumnName("id_uzytkownika");

                    b.Property<double?>("WartoscLiczbowaStatystyki")
                        .HasColumnType("float")
                        .HasColumnName("wartosc_liczbowa_statystyki");

                    b.Property<string>("WartoscStatystyki")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("wartosc_statystyki");

                    b.HasKey("Id");

                    b.HasIndex("DruzynaId");

                    b.HasIndex("RolaId");

                    b.HasIndex("StatystykaId");

                    b.HasIndex("UzytkownikId");

                    b.ToTable("Miejsce_w_druzynie", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Drużyny.Models.NastrojRozgrywki", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("nvarchar(40)")
                        .HasColumnName("nazwa");

                    b.HasKey("Id");

                    b.ToTable("Nastroj_Rozgrywki", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Platformy.Models.Platforma", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<byte[]>("Logo")
                        .IsRequired()
                        .HasColumnType("varbinary(max)")
                        .HasColumnName("logo");

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("nvarchar(40)")
                        .HasColumnName("nazwa");

                    b.HasKey("Id");

                    b.ToTable("Platforma", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Platformy.Models.UzytkownikPlatforma", b =>
                {
                    b.Property<int>("PlatformaId")
                        .HasColumnType("int")
                        .HasColumnName("id_platformy");

                    b.Property<int>("UzytkownikId")
                        .HasColumnType("int")
                        .HasColumnName("id_uzytkownika");

                    b.HasKey("PlatformaId", "UzytkownikId")
                        .HasName("id_uzytkownik_platforma");

                    b.HasIndex("UzytkownikId");

                    b.ToTable("Uzytkownik_platforma", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Powiadomienia.Models.Powiadomienie", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("DataWyslania")
                        .HasColumnType("datetime2")
                        .HasColumnName("data_wyslania");

                    b.Property<int?>("DrugiPowiazanyObiektId")
                        .HasColumnType("int")
                        .HasColumnName("id_drugiego_powiazanego_obiektu");

                    b.Property<string>("DrugiPowiazanyObiektNazwa")
                        .HasMaxLength(40)
                        .HasColumnType("nvarchar(40)")
                        .HasColumnName("nazwa_drugiego_powiazanego_obiektu");

                    b.Property<int?>("PowiazanyObiektId")
                        .HasColumnType("int")
                        .HasColumnName("id_powiazanego_obiektu");

                    b.Property<string>("PowiazanyObiektNazwa")
                        .HasMaxLength(40)
                        .HasColumnType("nvarchar(40)")
                        .HasColumnName("nazwa_powiazanego_obiektu");

                    b.Property<string>("Tresc")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)")
                        .HasColumnName("tresc");

                    b.Property<int>("TypPowiadomieniaId")
                        .HasColumnType("int")
                        .HasColumnName("id_typu_powiadomienia");

                    b.Property<int>("UzytkownikId")
                        .HasColumnType("int")
                        .HasColumnName("id_uzytkownika");

                    b.HasKey("Id");

                    b.HasIndex("TypPowiadomieniaId");

                    b.HasIndex("UzytkownikId");

                    b.ToTable("Powiadomienie", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Powiadomienia.Models.TypPowiadomienia", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("nazwa");

                    b.HasKey("Id");

                    b.ToTable("Typ_powiadomienia", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Nazwa = "Systemowe"
                        },
                        new
                        {
                            Id = 2,
                            Nazwa = "Zaproszenie do znajomych"
                        },
                        new
                        {
                            Id = 3,
                            Nazwa = "Zaakceptowano zaproszenie do znajomych"
                        },
                        new
                        {
                            Id = 4,
                            Nazwa = "Odrzucono zaproszenie do znajomych"
                        },
                        new
                        {
                            Id = 5,
                            Nazwa = "Usunieto cie ze znajomych"
                        });
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Jezyk", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("nazwa");

                    b.HasKey("Id");

                    b.ToTable("Jezyk", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Nazwa = "polski"
                        },
                        new
                        {
                            Id = 2,
                            Nazwa = "angielski"
                        },
                        new
                        {
                            Id = 3,
                            Nazwa = "niemiecki"
                        },
                        new
                        {
                            Id = 4,
                            Nazwa = "francuski"
                        },
                        new
                        {
                            Id = 5,
                            Nazwa = "hiszpański"
                        },
                        new
                        {
                            Id = 6,
                            Nazwa = "japoński"
                        },
                        new
                        {
                            Id = 7,
                            Nazwa = "rosyjski"
                        });
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.JezykProfilu", b =>
                {
                    b.Property<int>("JezykId")
                        .HasColumnType("int")
                        .HasColumnName("id_jezyka");

                    b.Property<int>("UzytkownikId")
                        .HasColumnType("int")
                        .HasColumnName("id_uzytkownika");

                    b.Property<int>("StopienBieglosciId")
                        .HasColumnType("int")
                        .HasColumnName("id_stopnia_bieglosci");

                    b.HasKey("JezykId", "UzytkownikId")
                        .HasName("id_jezyk_profilu");

                    b.HasIndex("StopienBieglosciId");

                    b.HasIndex("UzytkownikId");

                    b.ToTable("Jezyk_profilu", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Kraj", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("nazwa");

                    b.HasKey("Id");

                    b.ToTable("Kraj", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Nazwa = "Polska"
                        },
                        new
                        {
                            Id = 2,
                            Nazwa = "Anglia"
                        });
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Profil", b =>
                {
                    b.Property<int>("IdUzytkownika")
                        .HasColumnType("int")
                        .HasColumnName("id_uzytkownika");

                    b.Property<byte[]>("Awatar")
                        .HasColumnType("varbinary(max)");

                    b.Property<string>("Opis")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)")
                        .HasColumnName("opis");

                    b.Property<string>("Pseudonim")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("pseudonim");

                    b.Property<int?>("RegionId")
                        .HasColumnType("int");

                    b.Property<int>("StatusId")
                        .HasColumnType("int");

                    b.Property<string>("Zaimki")
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)")
                        .HasColumnName("zaimki");

                    b.HasKey("IdUzytkownika");

                    b.HasIndex("RegionId");

                    b.HasIndex("StatusId");

                    b.ToTable("Profil", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Region", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("KrajId")
                        .HasColumnType("int");

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("nazwa");

                    b.HasKey("Id");

                    b.HasIndex("KrajId");

                    b.ToTable("Region", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            KrajId = 1,
                            Nazwa = "Nie określono"
                        },
                        new
                        {
                            Id = 2,
                            KrajId = 1,
                            Nazwa = "Mazowieckie"
                        },
                        new
                        {
                            Id = 3,
                            KrajId = 1,
                            Nazwa = "Dolnoslaskie"
                        },
                        new
                        {
                            Id = 4,
                            KrajId = 1,
                            Nazwa = "Lubelskie"
                        },
                        new
                        {
                            Id = 5,
                            KrajId = 1,
                            Nazwa = "Lubuskie"
                        },
                        new
                        {
                            Id = 6,
                            KrajId = 1,
                            Nazwa = "Podkarpackie"
                        },
                        new
                        {
                            Id = 7,
                            KrajId = 1,
                            Nazwa = "Podlaskie"
                        },
                        new
                        {
                            Id = 8,
                            KrajId = 1,
                            Nazwa = "Zachodniopomorskie"
                        },
                        new
                        {
                            Id = 9,
                            KrajId = 1,
                            Nazwa = "Wielkopolskie"
                        },
                        new
                        {
                            Id = 10,
                            KrajId = 2,
                            Nazwa = "Nie określono"
                        },
                        new
                        {
                            Id = 11,
                            KrajId = 2,
                            Nazwa = "West Midlands"
                        },
                        new
                        {
                            Id = 12,
                            KrajId = 2,
                            Nazwa = "South West England"
                        },
                        new
                        {
                            Id = 13,
                            KrajId = 2,
                            Nazwa = "South East England"
                        },
                        new
                        {
                            Id = 14,
                            KrajId = 2,
                            Nazwa = "North West England"
                        },
                        new
                        {
                            Id = 15,
                            KrajId = 2,
                            Nazwa = "North East England"
                        },
                        new
                        {
                            Id = 16,
                            KrajId = 2,
                            Nazwa = "Greater London"
                        },
                        new
                        {
                            Id = 17,
                            KrajId = 2,
                            Nazwa = "East od England"
                        },
                        new
                        {
                            Id = 18,
                            KrajId = 2,
                            Nazwa = "East Midlands"
                        });
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Status", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("nazwa");

                    b.HasKey("Id");

                    b.ToTable("Status", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Nazwa = "Dostępny"
                        },
                        new
                        {
                            Id = 2,
                            Nazwa = "Zaraz wracam"
                        },
                        new
                        {
                            Id = 3,
                            Nazwa = "Nie przeszkadzać"
                        },
                        new
                        {
                            Id = 4,
                            Nazwa = "Niewidoczny"
                        },
                        new
                        {
                            Id = 5,
                            Nazwa = "Offline"
                        });
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.StopienBieglosciJezyka", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("nazwa");

                    b.Property<int>("Wartosc")
                        .HasColumnType("int")
                        .HasColumnName("wartosc");

                    b.HasKey("Id");

                    b.ToTable("Stopien_bieglosci_jezyka", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Nazwa = "Podstawowy",
                            Wartosc = 1
                        },
                        new
                        {
                            Id = 2,
                            Nazwa = "Średnio-zaawansowany",
                            Wartosc = 2
                        },
                        new
                        {
                            Id = 3,
                            Nazwa = "Zaawansowany",
                            Wartosc = 3
                        });
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.Kategoria", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("IdGry")
                        .HasColumnType("int")
                        .HasColumnName("id_gry");

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("nazwa");

                    b.HasKey("Id");

                    b.HasIndex("IdGry");

                    b.ToTable("Kategoria", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.Ranga", b =>
                {
                    b.Property<int>("StatystykaId")
                        .HasColumnType("int")
                        .HasColumnName("id_statystyki");

                    b.Property<int>("WartoscLiczbowa")
                        .HasColumnType("int")
                        .HasColumnName("wartosc_liczbowa");

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("nvarchar(40)")
                        .HasColumnName("nazwa");

                    b.HasKey("StatystykaId", "WartoscLiczbowa");

                    b.ToTable("Ranga", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.Rola", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("IdGry")
                        .HasColumnType("int");

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("nazwa");

                    b.HasKey("Id");

                    b.HasIndex("IdGry");

                    b.ToTable("Rola", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.Statystyka", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<bool>("CzyToCzasRozgrywki")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false)
                        .HasColumnName("czy_to_czas_rozgrywki");

                    b.Property<int>("KategoriaId")
                        .HasColumnType("int")
                        .HasColumnName("id_kategorii");

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("nazwa");

                    b.Property<int?>("RolaId")
                        .HasColumnType("int")
                        .HasColumnName("id_roli");

                    b.HasKey("Id");

                    b.HasIndex("KategoriaId");

                    b.HasIndex("RolaId");

                    b.ToTable("Statystyka", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.StatystykaUzytkownika", b =>
                {
                    b.Property<int>("UzytkownikId")
                        .HasColumnType("int")
                        .HasColumnName("id_uzytkownika");

                    b.Property<int>("StatystykaId")
                        .HasColumnType("int")
                        .HasColumnName("id_statystyki");

                    b.Property<double?>("PorownywalnaWartoscLiczbowa")
                        .HasColumnType("float")
                        .HasColumnName("porownywalna_wartosc_liczbowa");

                    b.Property<string>("Wartosc")
                        .HasMaxLength(40)
                        .HasColumnType("nvarchar(40)")
                        .HasColumnName("wartosc");

                    b.HasKey("UzytkownikId", "StatystykaId")
                        .HasName("id_statystyka_uzytkownika");

                    b.HasIndex("StatystykaId");

                    b.ToTable("Statystyka_uzytkownika", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.WymaganaStatystykaDruzyny", b =>
                {
                    b.Property<int>("DruzynaId")
                        .HasColumnType("int")
                        .HasColumnName("id_druzyny");

                    b.Property<int>("StatystykaId")
                        .HasColumnType("int")
                        .HasColumnName("id_statystyki");

                    b.Property<double?>("PorownywalnaWartoscLiczbowa")
                        .HasColumnType("float")
                        .HasColumnName("porownywalna_wartosc_liczbowa");

                    b.Property<string>("Wartosc")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("wartosc");

                    b.HasKey("DruzynaId", "StatystykaId");

                    b.HasIndex("StatystykaId");

                    b.ToTable("Wymagana_Statystyka_Druzyny", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateOnly?>("DataUrodzenia")
                        .HasColumnType("date")
                        .HasColumnName("data_urodzenia");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<int?>("IdNaZewnetrznymSerwisie")
                        .HasColumnType("int")
                        .HasColumnName("id_na_zewnetrznym_serwisie");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("LoginNaZewnetrznymSerwisie")
                        .HasMaxLength(40)
                        .HasColumnType("nvarchar(40)")
                        .HasColumnName("login_na_zewnetrznym_serwisie");

                    b.Property<string>("NormalizedEmail")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("NormalizedUserName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<DateTime?>("OstatniaAktywnosc")
                        .HasColumnType("datetime2")
                        .HasColumnName("ostatnia_aktywnosc");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .IsUnique()
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex");

                    b.ToTable("AspNetUsers", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Wiadomosci.Models.TypWiadomosci", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("nazwa");

                    b.HasKey("Id");

                    b.ToTable("Typ_wiadomosci", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Nazwa = "Prywatna"
                        },
                        new
                        {
                            Id = 2,
                            Nazwa = "Do gildii"
                        });
                });

            modelBuilder.Entity("Squadra.Server.Modules.Wiadomosci.Models.Wiadomosc", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("DataWyslania")
                        .HasColumnType("datetime2")
                        .HasColumnName("data_wyslania");

                    b.Property<int>("IdNadawcy")
                        .HasColumnType("int")
                        .HasColumnName("id_nadawcy");

                    b.Property<int>("IdOdbiorcy")
                        .HasColumnType("int")
                        .HasColumnName("id_odbiorcy");

                    b.Property<int>("IdTypuWiadomosci")
                        .HasColumnType("int");

                    b.Property<string>("Tresc")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)")
                        .HasColumnName("tresc");

                    b.HasKey("Id");

                    b.HasIndex("IdNadawcy");

                    b.HasIndex("IdTypuWiadomosci");

                    b.ToTable("Wiadomosc", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.WspieraneGry.Models.GraNaPlatformie", b =>
                {
                    b.Property<int>("IdWspieranejGry")
                        .HasColumnType("int")
                        .HasColumnName("id_gry");

                    b.Property<int>("IdPlatformy")
                        .HasColumnType("int")
                        .HasColumnName("id_platformy");

                    b.HasKey("IdWspieranejGry", "IdPlatformy")
                        .HasName("id_gra_na_platformie");

                    b.HasIndex("IdPlatformy");

                    b.ToTable("Gra_na_platformie", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.WspieraneGry.Models.WspieranaGra", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<string>("Gatunek")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)")
                        .HasColumnName("gatunek");

                    b.Property<string>("Tytul")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("nvarchar(60)")
                        .HasColumnName("tytul");

                    b.HasKey("Id");

                    b.ToTable("Wspierana_gra", (string)null);
                });

            modelBuilder.Entity("Squadra.Server.Modules.Znajomosci.Models.Znajomi", b =>
                {
                    b.Property<int>("IdUzytkownika1")
                        .HasColumnType("int")
                        .HasColumnName("id_uzytkownika_1");

                    b.Property<int>("IdUzytkownika2")
                        .HasColumnType("int")
                        .HasColumnName("id_uzytkownika_2");

                    b.Property<DateOnly>("DataNawiazaniaZnajomosci")
                        .HasColumnType("date")
                        .HasColumnName("data_nawiazania_znajomosci");

                    b.Property<DateTime?>("OstatnieOtwarcieCzatuUzytkownika1")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("OstatnieOtwarcieCzatuUzytkownika2")
                        .HasColumnType("datetime2");

                    b.HasKey("IdUzytkownika1", "IdUzytkownika2")
                        .HasName("id_znajomi");

                    b.HasIndex("IdUzytkownika2");

                    b.ToTable("Znajomi", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<int>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole<int>", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<int>", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<int>", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<int>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole<int>", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<int>", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Squadra.Server.Modules.BibliotekaGier.Models.GraUzytkownika", b =>
                {
                    b.HasOne("Squadra.Server.Modules.WspieraneGry.Models.WspieranaGra", "Gra")
                        .WithMany("GraUzytkownikaCollection")
                        .HasForeignKey("GraId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("GraUzytkownika_Gra");

                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", "Uzytkownik")
                        .WithMany("GraUzytkownikaCollection")
                        .HasForeignKey("UzytkownikId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("GraUzytkownika_Uzytkownik");

                    b.Navigation("Gra");

                    b.Navigation("Uzytkownik");
                });

            modelBuilder.Entity("Squadra.Server.Modules.BibliotekaGier.Models.GraUzytkownikaNaPlatformie", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Platformy.Models.Platforma", "Platforma")
                        .WithMany("GraUzytkownikaNaPlatformieCollection")
                        .HasForeignKey("PlatformaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("GraUzytkownikaNaPlatformie_Platforma");

                    b.HasOne("Squadra.Server.Modules.BibliotekaGier.Models.GraUzytkownika", "Gra")
                        .WithMany("GraUzytkownikaNaPlatformieCollection")
                        .HasForeignKey("UzytkownikId", "GraId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("GraUzytkownikaNaPlatformie_GraUzytkownika");

                    b.Navigation("Gra");

                    b.Navigation("Platforma");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Drużyny.Models.Druzyna", b =>
                {
                    b.HasOne("Squadra.Server.Modules.WspieraneGry.Models.WspieranaGra", "Gra")
                        .WithMany("DruzynaCollection")
                        .HasForeignKey("GraId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Druzyna_Gra");

                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", "Kapitan")
                        .WithMany("DowodzoneDruzynyCollection")
                        .HasForeignKey("KapitanId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Druzyna_Kapitan");

                    b.HasOne("Squadra.Server.Modules.Drużyny.Models.NastrojRozgrywki", "NastrojRozgrywki")
                        .WithMany("DruzynaCollection")
                        .HasForeignKey("NastrojRozgrywkiId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Druzyna_Nastroj_Rozgrywki");

                    b.HasOne("Squadra.Server.Modules.Platformy.Models.Platforma", "Platforma")
                        .WithMany("DruzynaCollection")
                        .HasForeignKey("PlatformaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("Druzyna_Platforma");

                    b.HasOne("Squadra.Server.Modules.Profile.Models.Jezyk", "WymaganyJezyk")
                        .WithMany("DruzynaCollection")
                        .HasForeignKey("WymaganyJezykId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("Druzyna_Wymagany_Jezyk");

                    b.HasOne("Squadra.Server.Modules.Profile.Models.StopienBieglosciJezyka", "WymaganyStopienBieglosciJezyka")
                        .WithMany("DruzynaCollection")
                        .HasForeignKey("WymaganyStopienBieglosciJezykaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("Druzyna_Wymagany_Stopien_Bieglosci_Jezyka");

                    b.Navigation("Gra");

                    b.Navigation("Kapitan");

                    b.Navigation("NastrojRozgrywki");

                    b.Navigation("Platforma");

                    b.Navigation("WymaganyJezyk");

                    b.Navigation("WymaganyStopienBieglosciJezyka");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Drużyny.Models.MiejsceWDruzynie", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Drużyny.Models.Druzyna", "Druzyna")
                        .WithMany("MiejsceWDruzynieCollection")
                        .HasForeignKey("DruzynaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Miejsce_W_Druzynie_Druzyna");

                    b.HasOne("Squadra.Server.Modules.Statystyki.Models.Rola", "Rola")
                        .WithMany("MiejsceWDruzynieCollection")
                        .HasForeignKey("RolaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("Miejsce_W_Druzynie_Rola");

                    b.HasOne("Squadra.Server.Modules.Statystyki.Models.Statystyka", "Statystyka")
                        .WithMany("MiejsceWDruzynieCollection")
                        .HasForeignKey("StatystykaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("Miejsce_W_Druzynie_Statystyka");

                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", "Uzytkownik")
                        .WithMany("MiejsceWDruzynieCollection")
                        .HasForeignKey("UzytkownikId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("Miejsce_W_Druzynie_Uzytkownik");

                    b.Navigation("Druzyna");

                    b.Navigation("Rola");

                    b.Navigation("Statystyka");

                    b.Navigation("Uzytkownik");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Platformy.Models.UzytkownikPlatforma", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Platformy.Models.Platforma", "Platforma")
                        .WithMany("UzytkownikPlatformaCollection")
                        .HasForeignKey("PlatformaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("UzytkownikPlatforma_Platforma");

                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", "Uzytkownik")
                        .WithMany("UzytkownikPlatformaCollection")
                        .HasForeignKey("UzytkownikId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("UzytkownikPlatforma_Uzytkownik");

                    b.Navigation("Platforma");

                    b.Navigation("Uzytkownik");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Powiadomienia.Models.Powiadomienie", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Powiadomienia.Models.TypPowiadomienia", "TypPowiadomienia")
                        .WithMany("PowiadomienieCollection")
                        .HasForeignKey("TypPowiadomieniaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Powiadomienie_TypPowiadomienia");

                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", "Uzytkownik")
                        .WithMany("PowiadomienieCollection")
                        .HasForeignKey("UzytkownikId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Powiadomienie_Uzytkownik");

                    b.Navigation("TypPowiadomienia");

                    b.Navigation("Uzytkownik");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.JezykProfilu", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Profile.Models.Jezyk", "Jezyk")
                        .WithMany("JezykProfiluCollection")
                        .HasForeignKey("JezykId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("JezykProfilu_Jezyk");

                    b.HasOne("Squadra.Server.Modules.Profile.Models.StopienBieglosciJezyka", "StopienBieglosciJezyka")
                        .WithMany("JezykProfiluCollection")
                        .HasForeignKey("StopienBieglosciId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("JezykProfilu_StopienBieglosci");

                    b.HasOne("Squadra.Server.Modules.Profile.Models.Profil", "Profil")
                        .WithMany("JezykProfiluCollection")
                        .HasForeignKey("UzytkownikId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("JezykProfilu_Profil");

                    b.Navigation("Jezyk");

                    b.Navigation("Profil");

                    b.Navigation("StopienBieglosciJezyka");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Profil", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", "Uzytkownik")
                        .WithOne("Profil")
                        .HasForeignKey("Squadra.Server.Modules.Profile.Models.Profil", "IdUzytkownika")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Squadra.Server.Modules.Profile.Models.Region", "Region")
                        .WithMany("ProfilCollection")
                        .HasForeignKey("RegionId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("Profil_Region");

                    b.HasOne("Squadra.Server.Modules.Profile.Models.Status", "Status")
                        .WithMany("ProfilCollection")
                        .HasForeignKey("StatusId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Profil_Status");

                    b.Navigation("Region");

                    b.Navigation("Status");

                    b.Navigation("Uzytkownik");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Region", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Profile.Models.Kraj", "Kraj")
                        .WithMany("RegionCollection")
                        .HasForeignKey("KrajId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Region_Kraj");

                    b.Navigation("Kraj");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.Kategoria", b =>
                {
                    b.HasOne("Squadra.Server.Modules.WspieraneGry.Models.WspieranaGra", "Gra")
                        .WithMany("KategoriaCollection")
                        .HasForeignKey("IdGry")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Gra");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.Ranga", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Statystyki.Models.Statystyka", "Statystyka")
                        .WithMany("RangaCollection")
                        .HasForeignKey("StatystykaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Statystyka");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.Rola", b =>
                {
                    b.HasOne("Squadra.Server.Modules.WspieraneGry.Models.WspieranaGra", "Gra")
                        .WithMany("RolaCollection")
                        .HasForeignKey("IdGry")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Gra");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.Statystyka", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Statystyki.Models.Kategoria", "Kategoria")
                        .WithMany("StatystykaCollection")
                        .HasForeignKey("KategoriaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Statystyka_Kategoria");

                    b.HasOne("Squadra.Server.Modules.Statystyki.Models.Rola", "Rola")
                        .WithMany("StatystykaCollection")
                        .HasForeignKey("RolaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("Statystyka_Rola");

                    b.Navigation("Kategoria");

                    b.Navigation("Rola");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.StatystykaUzytkownika", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Statystyki.Models.Statystyka", "Statystyka")
                        .WithMany("StatystykaUzytkownikaCollection")
                        .HasForeignKey("StatystykaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("StatystykaUzytkownika_Statystyka");

                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", "Uzytkownik")
                        .WithMany("StatystykaUzytkownikaCollection")
                        .HasForeignKey("UzytkownikId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("StatystykaUzytkownika_Uzytkownik");

                    b.Navigation("Statystyka");

                    b.Navigation("Uzytkownik");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.WymaganaStatystykaDruzyny", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Drużyny.Models.Druzyna", "Druzyna")
                        .WithMany("WymaganaStatystykaDruzynyCollection")
                        .HasForeignKey("DruzynaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("WymaganaStatystykaDruzyny_Druzyna");

                    b.HasOne("Squadra.Server.Modules.Statystyki.Models.Statystyka", "Statystyka")
                        .WithMany("WymaganaStatystykaDruzynyCollection")
                        .HasForeignKey("StatystykaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Wymagana_Statystyka_Druzyny_Statystyka");

                    b.Navigation("Druzyna");

                    b.Navigation("Statystyka");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Wiadomosci.Models.Wiadomosc", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", "Nadawca")
                        .WithMany("WiadomosciNadaneCollection")
                        .HasForeignKey("IdNadawcy")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Wiadomosc_Nadawca");

                    b.HasOne("Squadra.Server.Modules.Wiadomosci.Models.TypWiadomosci", "TypWiadomosci")
                        .WithMany("WiadomosciCollection")
                        .HasForeignKey("IdTypuWiadomosci")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Wiadomosc_Typ_Wiadomosci");

                    b.Navigation("Nadawca");

                    b.Navigation("TypWiadomosci");
                });

            modelBuilder.Entity("Squadra.Server.Modules.WspieraneGry.Models.GraNaPlatformie", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Platformy.Models.Platforma", "Platforma")
                        .WithMany("GraNaPlatformieCollection")
                        .HasForeignKey("IdPlatformy")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("GraNaPlatformie_Platforma");

                    b.HasOne("Squadra.Server.Modules.WspieraneGry.Models.WspieranaGra", "WspieranaGra")
                        .WithMany("GraNaPlatformieCollection")
                        .HasForeignKey("IdWspieranejGry")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("GraNaPlatformie_WspieranaGra");

                    b.Navigation("Platforma");

                    b.Navigation("WspieranaGra");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Znajomosci.Models.Znajomi", b =>
                {
                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", "Uzytkownik1")
                        .WithMany("ZnajomiJakoPierwszyCollection")
                        .HasForeignKey("IdUzytkownika1")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Znajomi_Uzytkownik1");

                    b.HasOne("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", "Uzytkownik2")
                        .WithMany("ZnajomiJakoDrugiCollection")
                        .HasForeignKey("IdUzytkownika2")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("Znajomi_Uzytkownik2");

                    b.Navigation("Uzytkownik1");

                    b.Navigation("Uzytkownik2");
                });

            modelBuilder.Entity("Squadra.Server.Modules.BibliotekaGier.Models.GraUzytkownika", b =>
                {
                    b.Navigation("GraUzytkownikaNaPlatformieCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Drużyny.Models.Druzyna", b =>
                {
                    b.Navigation("MiejsceWDruzynieCollection");

                    b.Navigation("WymaganaStatystykaDruzynyCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Drużyny.Models.NastrojRozgrywki", b =>
                {
                    b.Navigation("DruzynaCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Platformy.Models.Platforma", b =>
                {
                    b.Navigation("DruzynaCollection");

                    b.Navigation("GraNaPlatformieCollection");

                    b.Navigation("GraUzytkownikaNaPlatformieCollection");

                    b.Navigation("UzytkownikPlatformaCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Powiadomienia.Models.TypPowiadomienia", b =>
                {
                    b.Navigation("PowiadomienieCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Jezyk", b =>
                {
                    b.Navigation("DruzynaCollection");

                    b.Navigation("JezykProfiluCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Kraj", b =>
                {
                    b.Navigation("RegionCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Profil", b =>
                {
                    b.Navigation("JezykProfiluCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Region", b =>
                {
                    b.Navigation("ProfilCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.Status", b =>
                {
                    b.Navigation("ProfilCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Profile.Models.StopienBieglosciJezyka", b =>
                {
                    b.Navigation("DruzynaCollection");

                    b.Navigation("JezykProfiluCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.Kategoria", b =>
                {
                    b.Navigation("StatystykaCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.Rola", b =>
                {
                    b.Navigation("MiejsceWDruzynieCollection");

                    b.Navigation("StatystykaCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Statystyki.Models.Statystyka", b =>
                {
                    b.Navigation("MiejsceWDruzynieCollection");

                    b.Navigation("RangaCollection");

                    b.Navigation("StatystykaUzytkownikaCollection");

                    b.Navigation("WymaganaStatystykaDruzynyCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Uzytkownicy.Models.Uzytkownik", b =>
                {
                    b.Navigation("DowodzoneDruzynyCollection");

                    b.Navigation("GraUzytkownikaCollection");

                    b.Navigation("MiejsceWDruzynieCollection");

                    b.Navigation("PowiadomienieCollection");

                    b.Navigation("Profil");

                    b.Navigation("StatystykaUzytkownikaCollection");

                    b.Navigation("UzytkownikPlatformaCollection");

                    b.Navigation("WiadomosciNadaneCollection");

                    b.Navigation("ZnajomiJakoDrugiCollection");

                    b.Navigation("ZnajomiJakoPierwszyCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.Wiadomosci.Models.TypWiadomosci", b =>
                {
                    b.Navigation("WiadomosciCollection");
                });

            modelBuilder.Entity("Squadra.Server.Modules.WspieraneGry.Models.WspieranaGra", b =>
                {
                    b.Navigation("DruzynaCollection");

                    b.Navigation("GraNaPlatformieCollection");

                    b.Navigation("GraUzytkownikaCollection");

                    b.Navigation("KategoriaCollection");

                    b.Navigation("RolaCollection");
                });
#pragma warning restore 612, 618
        }
    }
}
