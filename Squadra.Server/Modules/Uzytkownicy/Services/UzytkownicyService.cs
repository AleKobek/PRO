...
public class UzytkownicyService(
    ...
    IProfileService profileService) : IUzytkownicyService
{
    ...
    public async Task<ServiceResult<UzytkownikResDto>> GetUzytkownik(int id)
    {
        try
        {
            return id < 1
                ...
                : ServiceResult<UzytkownikResDto>.Ok(await uzytkownicyRepository.GetUzytkownik(id));
        }
    ...
    public async Task<ServiceResult<UzytkownikResDto>> GetUzytkownik(string login)
    {
        try
        {
            return string.IsNullOrWhiteSpace(login)
                ...
                : ServiceResult<UzytkownikResDto>.Ok(await uzytkownicyRepository.GetUzytkownik(login));
        }
        catch (NieZnalezionoWBazieException e)
        {
            return ServiceResult<UzytkownikResDto>.NotFound(new ErrorItem(e.Message));
    ...
    public async Task<ServiceResult<DateTime?>> GetOstatniaAktywnoscUzytkownika(int id)
    {
        ...
        try
        {
            var ostatniaAktywnosc = await uzytkownicyRepository.GetOstatniaAktywnoscUzytkownika(id);
    ...
    public async Task<ServiceResult<bool>> CzyUzytkownikJestAdminem(int id)
    {
        ...
        try
        {
            return ServiceResult<bool>.Ok(await uzytkownicyRepository.CzyUzytkownikJestAdminem(id));
        }
        catch (NieZnalezionoWBazieException e)
        {
    ...
    public async Task<ServiceResult<bool>> CzyUzytkownikMaZintegrowaneKonto(int id)
    {
        ...
        try
        {
            var maZintegrowaneKonto = await uzytkownicyRepository.CzyUzytkownikMaZintegrowaneKonto(id);
    ...
    public async Task<ServiceResult<bool>> UpdateUzytkownik(int id, UzytkownikUpdateDto dto)
    {
        try
        {
            ...
            var result = await uzytkownicyRepository.UpdateUzytkownik(id, dto);
    ...
    public async Task<ServiceResult<bool>> UpdateHaslo(int idUzytkownika, string stareHaslo, string noweHaslo)
    {
        ...
        try
        {
            var wynikZBledami = await uzytkownicyRepository.UpdateHaslo(idUzytkownika, stareHaslo, noweHaslo);
    ...
    public async Task<ServiceResult<bool>> UpdateDaneKontaNaZewnetrznymSerwisie(int id, int? idNaZewnetrznymSerwisie, string? loginNaZewnetrznymSerwisie)
    {
        ...
        try
        {
            var result = await uzytkownicyRepository.UpdateDaneKontaNaZewnetrznymSerwisie(id, idNaZewnetrznymSerwisie, loginNaZewnetrznymSerwisie);
            return ServiceResult<bool>.Ok(result, 204);
        }
        catch (NieZnalezionoWBazieException e)
        {
    ...
    public async Task<ServiceResult<bool>> DeleteUzytkownik(int id)
    {
        ...
        try
        {
            ...
            var result = await profileService.DeleteProfil(id);
            if (!result.Succeeded) return result;
            
            await uzytkownicyRepository.DeleteUzytkownik(id);
        }catch(NieZnalezionoWBazieException e){
            return ServiceResult<bool>.NotFound(new ErrorItem(e.Message));
        }
        return ServiceResult<bool>.Ok(true, 204);
    }
    private async Task<bool> CzyLoginIstnieje(int? idUzytkownika, string login) => await uzytkownicyRepository.CzyLoginIstnieje(idUzytkownika, login);
    private async Task<bool> CzyEmailIstnieje(int? idUzytkownika, string email) => await uzytkownicyRepository.CzyEmailIstnieje(idUzytkownika, email);
