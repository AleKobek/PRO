...
public class RegionyRepository(AppDbContext context, IKrajeRepository krajeRepository) : IRegionyRepository
{
    ...
    public async Task<RegionKrajDto?> GetRegionIKraj(int? id)
    {
        ...
        if (region == null) throw new NieZnalezionoWBazieException("Region o id " + id + " nie istnieje");
        var kraj = await krajeRepository.GetKraj(region.KrajId);
