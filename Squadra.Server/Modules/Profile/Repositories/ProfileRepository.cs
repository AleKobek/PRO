...
public class ProfileRepository(AppDbContext appDbContext,
    IJezykiRepository jezykiRepository,
    IRegionyRepository regionyRepository,
    IStatusyRepository statusyRepository) : IProfileRepository
{
    ...
    public async Task<ProfilGetResDto> GetProfilUzytkownika(int id)
    {
        ...
        var jezykiUzytkownika = await jezykiRepository.GetJezykiProfilu(profil.IdUzytkownika);
        
        var region = profil.RegionId == null ? null : await regionyRepository.GetRegionIKraj(profil.RegionId);
        
        var status = await statusyRepository.GetStatus(profil.StatusId) ?? statusyRepository.GetStatusOffline();
    ...
    public async Task<bool> UpdateProfil(int id, ProfilUpdateDto profil)
    {
        ...
        // zmieniamy języki
        await jezykiRepository.ZmienJezykiProfilu(id, profil.Jezyki);
    ...
    public async Task<StatusDto> UpdateStatus(int id, int idStatus)
    {
        ...
        var status = await statusyRepository.GetStatus(idStatus);
