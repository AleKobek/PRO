...
[ApiController]
public class WspieraneGryController(
    ...
    UserManager<Uzytkownik> userManager) : ControllerBase
{
    ...
    public async Task<ActionResult<ICollection<WspieranaGraDto>>> GetWspieraneGry()
    {
        ...
        var result = await wspieraneGryService.GetWspieraneGry();
    ...
    public async Task<ActionResult<WspieranaGraDto>> GetWspieranaGra(int idGry)
    {
        ...
        var result = await wspieraneGryService.GetWspieranaGra(idGry);
        return result.StatusCode switch
        {
            200 => Ok(result.Value),
    ...
    public async Task<ActionResult> GetWspieraneGryMinInfo()
    {
        ...
        var result = await wspieraneGryService.GetWspieraneGryMinInfo();
    ...
    public async Task<ActionResult<ICollection<GraZPlatformaDTO>>> GetWspieraneGryZPlatformami()
    {
        ...
        var result = await wspieraneGryService.GetWspieraneGryZPlatformami();
        return result.StatusCode == 200
            ? Ok(result.Value)
            : StatusCode(result.StatusCode, new { errors = result.Errors });
    }
    ...
    public async Task<ActionResult<ICollection<Platforma>>> GetPlatformyGry(int idGry)
    {
        ...
        var result = await wspieraneGryService.GetPlatformyGry(idGry);
        return result.StatusCode switch
        {
            200 => Ok(result.Value),
            400 => BadRequest(result.Errors[0].Message),
