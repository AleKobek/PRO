...
public class WspieraneGryService(IWspieraneGryRepository wspieraneGryRepository) : IWspieraneGryService
{
    public async Task<ServiceResult<ICollection<WspieranaGraDto>>> GetWspieraneGry()
    {
        var gry = await wspieraneGryRepository.GetWspieraneGry();
    ...
    public async Task<ServiceResult<WspieranaGraDto>> GetWspieranaGra(int idGry)
    {
        try
        {
            ...
            var gra = await wspieraneGryRepository.GetWspieranaGra(idGry);
    ...
    {
        return ServiceResult<ICollection<GraZPlatformaDTO>>.Ok(await wspieraneGryRepository.GetWspieraneGryZPlatformami());
    }
    ...
    {
        var gryZPlatformami = await wspieraneGryRepository.GetWspieraneGryZPlatformami();
    ...
    public async Task<ServiceResult<ICollection<PlatformaDto>>> GetPlatformyGry(int idGry)
    {
        try
        {
            ...
            var platformyRes = await wspieraneGryRepository.GetPlatformyGry(idGry);
    ...
    public async Task<ServiceResult<ICollection<PlatformaDto>>> GetPlatformyGryUzytkownika(int idGry, int idUzytkownika)
    {
        try
        {
            ...
            var platformyRes = await wspieraneGryRepository.GetPlatformyGryUzytkownika(idGry, idUzytkownika);
            return ServiceResult<ICollection<PlatformaDto>>.Ok(platformyRes.Select(x => new PlatformaDto(x.Id, x.Nazwa, x.Logo)).ToList());
        }
        catch (NieZnalezionoWBazieException e)
        {
